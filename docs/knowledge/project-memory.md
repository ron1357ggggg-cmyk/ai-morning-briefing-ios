...
- 健康與新聞其中一方失敗時，另一方成功取得的資料仍必須更新。
- 測試 fixture 必須固定、可重現，不依賴即時網路或目前日期的偶然結果。
- 不可用自動 retry 隱藏不穩定測試；失敗時需保留輪次與結果檔。
- 使用者要求壓力或重複驗證時，執行 `./scripts/repeat-tests.sh 101`。
- 本次 iPhone 17 模擬器完成 101 輪、1,313 次測試且 0 失敗，結果檔位於本機
  `build/test-results`，該目錄不提交 Git。
- 自動測試不能保證零缺陷；實機 HealthKit 資料完整性、RSS 來源可用性及 iOS
  背景執行限制仍需持續觀察。
